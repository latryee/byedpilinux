...
var AvailableStrategies = map[string]Strategy{
	...
	"strategy_c": {
		...
		Description:     "Splits the TLS ClientHello into two TCP segments (first 2 bytes). Recommended for Turkish ISPs.",
		RequiresBackend: true,
		RequiresDoH:     true,
		NFQWSArgs:       []string{"--dpi-desync=split2"},
		NativeSplitPos:  2,
	},
	"strategy_d": {
		ID:              "strategy_d",
		Name:            "Advanced DPI Desync (fake + split2)",
		Description:     "Sends a low-TTL fake TLS ClientHello before real segmented packet. Evades stateful middleboxes.",
		RequiresBackend: true,
		RequiresDoH:     true,
		NFQWSArgs:       []string{"--dpi-desync=fake,split2", "--dpi-desync-ttl=4"},
		NativeSplitPos:  2,
	},
}
